                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_tag_0**: every element of $.message.order.fulfillments[*].tags[*].code must be in ["state", "linked_provider", "linked_order", "linked_order_item"]
	...
	> - **condition B**: $.message.order.fulfillments[*].tags[*].code must **not** be present in the payload`,
        ...
        function validate_tag_0_state(
            ...
            for (const testObj of scope) {
                ...
                const validValues = ["ready_to_ship", "order_ready"];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_tag_0_linked_provider**: every element of $.message.order.fulfillments[*].tags[?(@.code=='linked_provider')].list[*].code must be in ["id", "name", "address"]`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_tag_0_linked_order**: every element of $.message.order.fulfillments[*].tags[?(@.code=='linked_order')].list[*].code must be in ["id", "currency", "declared_value", "weight_unit", "weight_value", "dim_unit", "length", "breadth", "height", "shipment_type"]`,
                        },
                    ];
                }
        ...
        function validate_tag_0_linked_order_item(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(
                input.payload,
