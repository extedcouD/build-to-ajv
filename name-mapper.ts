...
const data = {
	search: [
		...
		},
		{
			...
			validValues: ["BUYER_FINDER_FEES_PERCENTAGE", "BUYER_FINDER_FEES_AMOUNT"],
		...
		{
			_NAME_: "validate_tag_1_PAGINATION",
	...
	select: [
		...
		},
		{
			_NAME_: "validate_enum_11",
			enumList: ["UNCLAIMED", "CLAIMED", "EXPIRED"],
			enumPath: "$.message.order.fulfillments[*].stops[*].authorization.status",
			_CONTINUE_: "!(enumPath are present)",
			_RETURN_: "enumPath all in enumList",
		},
		{
	...
	on_search: [
		...
		},
		{
			...
			enumPath: "$.message.catalog.providers[*].categories[*].descriptor.code",
			_CONTINUE_: "!(enumPath are present)",
			_RETURN_: "enumPath all in enumList",
		},
		{
		...
		},
		{
			_NAME_: "validate_enum_10",
			enumList: ["START", "END", "INTERMEDIATE_STOP", "TRANSIT_STOP"],
			enumPath: "$.message.catalog.providers[*].fulfillments[*].stops[*].type",
			_CONTINUE_: "!(enumPath are present)",
			_RETURN_: "enumPath all in enumList",
		},
		{
		...
		{
			_NAME_: "validate_tag_4_PAGINATION",
			_SCOPE_: "$.message.catalog.tags[?(@.descriptor.code=='PAGINATION')]",
			subTags: "$.list[*].descriptor.code",
			validValues: ["PAGINATION_ID", "CURRENT_PAGE_NUMBER", "MAX_PAGE_NUMBER"],
			_RETURN_: "subTags all in validValues",
		},
	],
	on_select: [
...
function removeArrayMarkers(input: string): string {
	return input.replace(/\[\*?\]/g, "");
}

for (const api in data) {
	let i = 0;
	for (const test in data[api]) {
		for (const key in data[api][test]) {
			let new_name = "";
			if ("attr" in data[api][test]) {
				new_name = removeArrayMarkers(data[api][test].attr.split(".").pop());
			}
			if (key === "_NAME_") {
				if (new_name === "") continue;
				const gen = `validate_${new_name}_${i}`;
				i++;
				data[api][test][key] = gen;
			}
		}
	}
}
writeFileSync("clean.json", JSON.stringify(data, null, 2));
